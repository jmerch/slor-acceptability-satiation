...
all_experiments = rbind(all_experiments, satiation)
...
plot = CIs %>%
  ...
  theme(legend.title=element_blank())+
  theme(legend.text=element_text(size=12))+
  geom_text(aes(x=2.55,y=0.55, label = "whether-island"), size=3.5, colour = "#FF699C", hjust = 0, vjust = 0, check_overlap = TRUE )+
  geom_text(aes(x=2.25,y=0.4, label = "subject island"), size=3.5, colour = "#DC72FB", hjust = 0, vjust = 0, check_overlap = TRUE ) +
  geom_text(aes(x=2.65,y=0.475, label = "CNPC"), size=3.5, colour = "#F8766D", hjust = 0, vjust = 0, check_overlap = TRUE )
plot
ggsave(filename = 'ALL_SLOR_surp_error_bars_right.png', path="plots/lu_kim_2022", width=8, height=5)
